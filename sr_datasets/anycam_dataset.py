...
class AnycamDataset(GenericMVSDataset):
    """
    Reads a Anycam scan folder.
        ...
        if os.path.exists(valid_frame_path):
            ...
            if store_computed:
                ...
                try:
                    ...
                    print(
                        f"Couldn't save valid_frames at {valid_frame_path}, " f"cause:"
                    )
                    print(e)
    ...
    def load_pose(self, scan_id, frame_id):
        ...
        world_T_cam = (
            torch.tensor(frame_metadata["pose4x4"], dtype=torch.float32).view(4, 4).T
        )
    ...
    def load_target_size_depth_and_mask(self, scan_id, frame_id):
        ...
        if os.path.exists(confidence_filepath):
            ...
            confidence_filepath = self.get_full_res_confidence_filepath(
                scan_id, frame_id
            )
            conf = torch.from_numpy(
                np.fromfile(confidence_filepath, dtype=np.uint8).reshape(
            ...
            conf = F.interpolate(
                conf, size=(self.depth_height, self.depth_width), mode="nearest"
            )
    ...
    def get_color_filepath(self, scan_id, frame_id):
        ...
        cached_resized_path = os.path.join(
            scene_path, f"frame.{self.image_width}_{frame_id}.jpg"
        )
